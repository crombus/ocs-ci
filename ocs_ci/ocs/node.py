...
def add_new_node_and_label_upi(config, num_nodes, mark_for_ocs_label=True):
    """
    Add a new node for aws upi platform and label it

    Args:
        config (dict): To check if platform is aws upi rhel or rhcos
    ...
    if config.ENV_DATA.get('rhel_workers'):
        logging.info('AWS UPI RHEL adding node')
        node_util.create_and_attach_nodes_to_cluster(
            {}, 'RHEL', num_nodes)
    else:
        logging.info('AWS UPI RHCOS adding node')
        node_util.create_and_attach_nodes_to_cluster(
            {}, 'RHCOS', num_nodes)
